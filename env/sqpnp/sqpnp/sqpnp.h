...
namespace sqpnp
{
  ...
  private:
    ...
    // Pseudoinverse of a 3x3 matrix using SVD (Eigen versions without COD support)
    inline static void PseudoInverse3x3(const Eigen::Matrix<double, 3, 3>& M,
                                        Eigen::Matrix<double, 3, 3>& Minv)
    {
      Eigen::JacobiSVD<Eigen::Matrix<double, 3, 3>> svd(
          M, Eigen::ComputeFullU | Eigen::ComputeFullV);
      const Eigen::Matrix<double, 3, 1> S = svd.singularValues();
      const double max_s = S.maxCoeff();
      const double tol = 3.0 * std::numeric_limits<double>::epsilon() * max_s;
      Eigen::Matrix<double, 3, 1> Sinv;
      Sinv[0] = (S[0] > tol) ? (1.0 / S[0]) : 0.0;
      Sinv[1] = (S[1] > tol) ? (1.0 / S[1]) : 0.0;
      Sinv[2] = (S[2] > tol) ? (1.0 / S[2]) : 0.0;
      Minv = svd.matrixV() * Sinv.asDiagonal() * svd.matrixU().transpose();
    }
					 ...
					 )
    {
      ...
      if ( fabs(det) < det_threshold ) { PseudoInverse3x3(Q, Qinv); return false; } // fall back to pseudoinverse
